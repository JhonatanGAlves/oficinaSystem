...
<body>
	...
	<h2>Cadastro Agenda</h2>
		...
		<?php } ?>
    	<form action="php/cad_age.php" method="POST">
		    <div class="form-group">
		    	<input type="text" name="hora" autofocus placeholder="Digite a hora" maxlength="5" onkeypress="formatar('##:##', this)" class="form-control" > 
		    </div>
		    <br>
		    <div class="form-group">
		    	<input type="text" name="data" autofocus maxlength="10" onkeypress="formatar('##/##/####', this)" placeholder="Digite a data" class="form-control">
		    </div>
		    <br>
		    <div class="form-group">
		    	<select class="form-select" aria-label="select" name="select">
		    		...
		    		<?php
		    			$result_cli = "SELECT * FROM cliente";
		    			$resultado_cli = mysqli_query($conn, $result_cli);
		    			while($row_cli = mysqli_fetch_assoc($resultado_cli)){ ?>
		    				<option value="<?php echo $row_cli['idCliente']; ?>"><?php echo $row_cli['nomeCliente']; ?>
		    				</option><?php
		    			}
		    		?>
		    	</select>
    		...
    		<?php if(mysqli_num_rows($result)){ ?>
	    		<table class="table table-bordered table-hover">
	    			...
	    			<tbody>
	    				...
	    				<tr  align="center">
	    					<td scope="row"><?=$i?></td>
	    					<td><?=$rows['idAgendamento']?></td>
	    					<td><?php echo $rows['horarioAgendamento']?></td>
	    					<td><?php echo $rows['dataAgendamento']?></td>
	    					<td><a class="link-success" href="cliente.php"><?php echo $rows['idCliente']?></a></td>
	    					<td>
	    						<a href="update_age.php?id=<?=$rows['idAgendamento']?>"
	    							class="btn btn-outline-warning">Alterar</a>
	    						<a href="php/delete_age.php?id=<?=$rows['idAgendamento']?>"
